...
def test_import_activate_train_and_dashboard_flow():
    content = """employee_id,department,gender,age,salary,hire_date,training_hours,performance_score,satisfaction_score,attrition,full_name
...
"""
    response = client.post(
        "/api/imports/csv",
        files={"file": ("valid.csv", BytesIO(content.encode("utf-8")), "text/csv")},
    )
    assert response.status_code == 200
    import_id = response.json()["importId"]
    assert response.json()["validationSummary"]["invalid_rows"] == 0

    activate_response = client.post(f"/api/imports/{import_id}/activate")
    ...
    train_response = client.post("/api/models/attrition/train")
    assert train_response.status_code == 200
    assert "metrics" in train_response.json()

    predictions_response = client.get("/api/models/attrition/predictions")
