...
def train_attrition_model(db: Session) -> dict:
    ...
    class_counts = dataframe["attrition"].value_counts()
    if class_counts.min() < 2:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Chaque classe de départ doit contenir au moins deux employés pour entraîner le modèle.",
        )
    ...
    confusion = confusion_matrix(y_test, test_predictions, labels=[0, 1]).tolist()
